...
那我们只要想办法弄到 `\x0f` 和 `\x05` 就成功了一半。观察内存，发现有一个现成的 `\x05`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.2rvmves3vy.avif)


虽然也有现成的 `\x0f`，但是它行吗？我们可以做一个简单的测试，直接找一片空内存改，然后看看解析出来是什么指令：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.32igoksjy3.avif)


并不是我们期望的 `syscall`，很简单，因为 `amd64` 是小端序的，所以我们不能写 `\x0f`，而是应该写 `0x0f00000000000000`。


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.99tuor2zt2.avif)
...
至于为啥必须这样？因为我的想法是找一个带 `\x0f` 的 `push` or `pop` 指令放在最后，然后用一堆单字节的 `push` or `pop` 将 `\x0f` 卡到第八个字节的位置，最后将事先获取到的 `\x05` 通过 `push` 覆盖掉前面被挤出来的字节，就有了一个 `syscall`。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.5triwpchiq.avif)
...
这里选的指令是 `pop fs`，实测 `push fs` 不行。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.7lkhrm6jvb.avif)


所以我的 exp 就不难理解了，一开始的 `0x4d` 个 `pop r15` 是为了弄到 `\x05`，保存在 `r15` 里：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.6f16izwiop.avif)
...
先看一下最终的调用链：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.1zirgqfcjo.avif)
...
那现在问题就变成了，如何控制 `rcx` 指向 `system`？调试发现，`rcx` 的计算过程是可逆的，并且可以控制为任意值。具体流程，需要从 `jmp rcx` 开始反向溯源，看它是怎么得来的。最终发现，源头来自执行 [\_IO_wdo_write](https://sourcegraph.com/github.com/bminor/glibc@fb4db64a04ad6c96cd1fbb7e02eb59323b1f2ac2/-/blob/libio/libioP.h?L566) 时的 `lea rcx, [r12 + r13*4]`，`rcx` 从这里被设置后直到执行 `__fct` 都没有被修改过。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.7axo1vqy3o.avif)
...
但是直接这样设置发现，并没有得到 `system`，于是我们继续往上溯源，看一下 `rsi` 和 `rdx` 到底是怎么传入的，发现，`rdx` 其实是被动过手脚的……
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.32igs2x3uo.avif)
