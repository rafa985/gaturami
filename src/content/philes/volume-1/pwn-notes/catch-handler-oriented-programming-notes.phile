...
首先是只覆盖了 `rbp`，我们发现执行 `_Unwind_Resume` 前 `rbp` 是 `0x7ffe29f72c50 ◂— 0x4242424242424242 ('BBBBBBBB')`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.2ksaalfwn4.avif)


执行完 `_Unwind_Resume` 后 `rbp` 就变成了我们写入的 `B`，然后继续往下执行，最终卡在了这个地方：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.pfphz109v.avif)
...
既然 rbp 只要是能 rw 的地址，程序又没开 PIE，那我们直接将其修改为 bss 的地址，就可以继续往下执行了：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.6f11tkdlft.avif)


现在我们尝试覆盖返回地址，看看会发生什么：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.szbfpmqwq.avif)


又是因为访问非法地址而 abort，这次是 `rax`，而 `rax` 存的是我们覆盖的返回地址。嗯……如果继续用 bss 地址代替会发生什么呢？


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.45i1a3etar.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.6m49p0rvc0.avif)
