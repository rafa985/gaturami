...
这里我们在调用 `gets` 的地方下断点，执行 `gets` 之前 `rdi` 指向的是 buf 的栈地址，`ni`，随便输入什么后，发现 `rdi` 寄存器变成了 `*RDI  0x7ffff7e137c0 (_IO_stdfile_0_lock) ◂— 0`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.1lc6yoffsz.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.13m5a38r8s.avif)
...
很关键的一点就是，`_IO_release_lock(_fp)` 也属于这个定义域，所以如果 `_IO_release_lock(_fp)` 返回了，也会自动调用上面设置的 cleanup 函数。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.96a43f63z9.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.7pnv6o8tk.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.esvqmae96.avif)
...
```
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.9o05sn4ed6.avif)
...
可以看下图，是第二次 gets 后将 `cnt` 填充为垃圾字节后的结果，此时 `owner` 还持有着一个地址，但并不是 TLS 地址（虽然但是，这并不妨碍我们接下来泄漏 TLS）：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.7p3z2o4g2j.avif)
...
那么我们输入四个 junk value padding 掉 `lock` 就会导致 `cnt` 低位变成 `\x00`，如下图：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.4qroz5qju4.avif)


但是紧接着就会将 `cnt` 自减，`\x00` 变成 `\xff`：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.1lc708fw6z.avif)
...
没毛病，rdi 的值还残留着 `_IO_2_1_stdout_`，成功令它变成可写地址，那现在就可以像上面一样使用 `gets` 了。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.3nrzsf095k.avif)
...
此外，根据引用我们推测，下面这些函数可能也存在同样的问题，不过还是需要自己去看代码才能确定。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.5treegc8sj.avif)
