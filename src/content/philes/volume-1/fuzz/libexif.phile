...
这个 Gemini Cam 还挺滑稽的哈哈哈：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.8s3ud0wapg.avif)
...
hang 的情况比较唯一，全是卡在 `exif_loader_write` 这里：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.39lpzh3w27.avif)
...
`exif_loader_get_data` 负责为 `ExifData` 结构体开辟空间，然后使用 `exif_data_load_data` 将内存中的 JPEG / EXIF _header_, _byte order_, _fixed value_ 等信息加载到 ExifData 结构体中，进行一些简单的校验，并初始化 _IFD 0_ 和 _IFD 1_。初始化 IFD 字段是通过调用 `exif_data_load_data_content` 实现的。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.92qoah6kym.avif)
...
首先确定一下在第几次循环的时候崩溃，我们直接 `c` 让它崩，然后切换到对应栈帧查看 `i`，发现是第 13 次调用了 `exif_data_load_data_thumbnail`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.9kgpz2zpfk.avif)
...
然后通过 `if i == 13` 下条件断点，定位到触发崩溃的循环索引继续分析。由于这里有两个调用 `exif_data_load_data_thumbnail` 的位置，而我们不知道具体会调用哪一个，因此如果我们下的那个断点不是实际的调用点，那就停不下来了。所以这里两个位置都需要下一个断点：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.8ok8jnphmq.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.7w7d1x7eu9.avif)


最后我们发现它执行的是 381 行处的代码，整洁起见，之前 374 行处的断点就可以删掉了。


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.32ii5su2d6.avif)


步入继续，发现致使 `memcpy` 崩溃的原因是 `rsi` 无法解引用：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.6po1tbt9wu.avif)
...
即将 _size_ 设置为 `r8`，将 _data_ 设置为 `exif_data_alloc` 分配出来的值，而 _src_ 则是 `d + offset` 的值，即 `rsi + 0xffffff80`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.icnt6cnbs.avif)
...
回溯发现，在给 `exif_data_load_data_thumbnail` 传参时，offset 的值就有问题了：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.6f18073lot.avif)


通过查看二进制数据，我们很容易定位到一个类似的数据：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.67y04sub0g.avif)


尝试直接将其修改为 `0xCAFEBABE`，然后再调试一下。调试之前我们可以先使用 `save breakpoints bps` 将断点信息保存到 `bps` 文件，之后通过 `source bps` 加载。


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.3yezlbj28h.avif)
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.4clfc6tir1.avif)
...
其实我觉得没必要从 main 开始分析，直接从崩溃边界开始分析就好了，所以接下来我会从后往前分析。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.2a5mqzim92.avif)
...
回溯到调用 `exif_get_long` 的函数 `exif_entry_fix`：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.1apjdtv699.avif)


我们发现这里是一个 for 循环，并且此时的索引是 `31440`，巨大无比，显然不正常，而索引范围是 `e->components` 指定的：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.1zisxut0p7.avif)


这都十亿多次了，所以问题就是没有验证这个 _component_ 大小的合法性。此外，这个大小也是攻击者可控的：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.2yywb0xnz3.avif)


_components_ 肯定代表了组件数量，什么东西的组件数量？翻代码，发现是 `ExifEntry`：


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.26m0tar13e.avif)
...
通过查询 exif 标准 [^1] 我们可知，一个 `ExifEntry` 由 `Format`，`Components` 和 `Data` 组成。如果 _format_ 类型是 `unsigned short`，则它代表每个 _component_ 占用 2 bytes，而 _components_ 则代表了这样的组件有几个，将它和 _format_ 相乘即是这一 _entry_ 总共占用的字节数。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.32ii8rjv20.avif)
...
调试发现，这个非法 offset 来自 `exif_get_long` 函数，我们深入分析一下它的逻辑。
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.5trkdyugrn.avif)
...
成功修复：
  ...
  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.szhpf1sld.avif)


但是发现 rational 类型也有问题，不过直接使用和上面一样的 patch 方案就能解决。


  ![](https://v6.gh-proxy.org/https://github.com/CuB3y0nd/picx-images-hosting/raw/master/.2rvofrnxvl.avif)
